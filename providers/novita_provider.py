"""
Novita TTS Provider
Implementation for Novita AI Text-to-Speech API (MiniMax Speech 2.8 Turbo)
"""
...
class NovitaProvider(BaseTTSProvider):
    """Novita AI Text-to-Speech provider implementation"""

    DEFAULT_API_URL = "https://api.novita.ai/v3/minimax-speech-2.8-turbo"
    ...
    def text_to_speech(self, text: str, output_path: str, voice_settings: Optional[Dict[str, Any]] = None) -> bool:
        """
        Convert text to speech using Novita API
        ...
        # Build request payload (same format as MiniMax)
        payload = {
            "text": text,
            "stream": False,
        ...
        try:
            print(f"🔊 Generating audio ({len(text)} characters) with Novita...")
    ...
    def get_available_voices(self) -> Dict[str, Any]:
        """
        Get available Novita voices (same as MiniMax)
    ...
    def get_default_voice_settings(self) -> Dict[str, Any]:
        """
        Get default Novita voice settings
    ...
    def validate_settings(self, voice_settings: Dict[str, Any]) -> bool:
        """
        Validate Novita voice settings
